    ...
    var Game = Class.extend({
        ...
        initAchievements: function() {
            ...
            this.achievements = {
                ...
                SMALL_TALK: {
                    id: 4,
                    name: "Spoke with an AI Oracle",
                    desc: "Talk to an AI-powered NPC in Hallow"
                },
                FAT_LOOT: {
                    id: 5,
                    name: "Fat Loot",
        ...
        connect: function(started_callback) {
            ...
            this.client.onWelcome(function(id, name, x, y, hp) {
                ...
                if(!self.storage.hasAlreadyPlayed()) {
                    ...
                    self.showNotification("Welcome to Hallow!");
                    if(window.hallowGameStarted) window.hallowGameStarted();
                } else {
                    self.showNotification("Welcome back to Hallow!");
                    if(window.hallowGameStarted) window.hallowGameStarted();
                            ...
                            if(entity.id !== self.playerId) {
                                ...
                                if(entity instanceof Character) {
                                    ...
                                    entity.onDeath(function() {
                                        ...
                                        if(window.hallowRegisterKill) window.hallowRegisterKill();
                                        }
                ...
                self.client.onPlayerKillMob(function(kind) {
                    ...
                    if(kind === Types.Entities.BOSS) {
                        ...
                        setTimeout(function() {
                            if(window.hallowShowWinScreen) {
                                var weaponName = self.player ? self.player.getWeaponName() : "Unknown";
                                window.hallowShowWinScreen(weaponName);
                            }
                        }, 2000);
                    }
                });
        ...
        makeNpcTalk: function(npc) {
            ...
            if(npc) {
                // AI NPCs: priest and sorcerer get Claude AI responses
                var aiNpcs = ["priest", "sorcerer"];
                var npcKind = npc.itemKind || "";

                if(aiNpcs.indexOf(npcKind) !== -1 && npc.talkIndex === 0) {
                    // Show a loading bubble first
                    self.createBubble(npc.id, "...");
                    ...
                    // Call Claude AI
                    var playerName = self.player ? self.player.name : "traveller";
                    var kills = window.hallowKills || 0;
                    var npcPersona = npcKind === "priest"
                        ? "You are an ancient mystical priest in the dark fantasy world of Hallow. You give wise, cryptic guidance to adventurers. Keep it under 15 words."
                        : "You are a powerful sorcerer in the dark world of Hallow. You are mysterious and boastful. Keep it under 15 words.";
                    ...
                    fetch("/api/npc-chat", {
                        ...
                        body: JSON.stringify({
                            persona: npcPersona,
                            message: "The adventurer " + playerName + " who has slain " + kills + " enemies approaches you. Greet them with one short in-character line."
                        })
                    })
                    ...
                    .then(function(data) {
                        var aiMsg = (data.text)
                            ...
                            : null;
                        if(aiMsg) {
                            self.createBubble(npc.id, aiMsg);
                            self.assignBubbleTo(npc);
                        } else {
                            // No API key configured or error: fall back to lore
                            msg = npc.talk();
                            if(msg) {
                        ...
                        npc.talkIndex = 1; // move past first line next time
                    })
                    .catch(function() {
                        // Fallback to original dialogue on error
                        msg = npc.talk();
                        if(msg) {
                            self.createBubble(npc.id, msg);
                            self.assignBubbleTo(npc);
                    ...
                    if(npc.kind === Types.Entities.RICK) {
                        self.tryUnlockingAchievement("RICKROLLD");
                    }
                    return;
                }

                // Default behaviour for all other NPCs
                msg = npc.talk();
                this.previousClickPosition = {};
                if(msg) {
